...
# ============================================================
# How to use:
# python Analyze_Splitting_v3.py --prefix "AM_{u}_L" --u-min 0 --u-max 9 --emin -2.0 --emax 0.5 --topn 10 --percentile 95 --out-prefix L_path_summary
#
# Example output files:
#   L_path_summary.csv
#   L_path_summary.png
#
# This script analyses spin splitting from spin-polarised CASTEP
# .bands files, using an energy window defined by --emin and --emax
# (relative to EF because zero_fermi=True is used).
# ============================================================
...
def make_plot(rows, out_png, args):
    ...
    plt.plot(
        u_vals, pct_vals,
        marker="^",
        label=f"{args.percentile:g}th Percentile |ΔE|"
    )
    plt.plot(
        u_vals, topn_vals,
        marker="d",
        label=f"Top-{args.topn} Mean |ΔE|"
    )

    plt.xlabel("Hubbard U (eV)")
    plt.ylabel("Energy (eV)")
    plt.title(
        f"Band Gap and Spin Splitting vs Hubbard U\n"
        f"Window = [{args.emin}, {args.emax}] eV"
    )
    plt.legend()
    plt.tight_layout()
    ...
    for u in range(args.u_min, args.u_max + 1):
        ...
        try:
            ...
            print(
                ...
                f"top{args.topn}={row['topn_mean_split_eV']:.6f} eV, "
